...
export default function Home() {
  ...
  const { data: products, refetch } = useQuery({
    queryKey: ["products"],
    queryFn: async () => {
      const { data } = await axios.post<QueryResult<TProduct>[]>('https://ecommerce-filtering-solution.vercel.app/api/products', {
        filter: {
          sort: filter.sort,
          color: filter.color,
                      ...
                      SIZE_FILTERS.options.map((option, optionIdx) => (
                        <li key={option.value} className="flex items-center">
                          <input onChange={() => {
                            ...
                            type="checkbox" id={`size-${optionIdx}`} className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
